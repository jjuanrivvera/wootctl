...
func init() {
	metaRegistrars = append(metaRegistrars, func(d *deps) *cobra.Command {
		return &cobra.Command{
			...
			Long: `Walk through connecting wootctl to a Chatwoot instance: base URL, api_access_token
...
init is auth login plus a doctor pass; re-running it is safe.`,
			Example: `  wootctl init
  wootctl --profile staging init`,
			Args: cobra.NoArgs,
			RunE: func(cmd *cobra.Command, _ []string) error {
				fmt.Fprintln(cmd.ErrOrStderr(), "wootctl setup — connect to your Chatwoot instance")
				...
				offerChatwootDropIn(cmd)
				fmt.Fprintln(cmd.ErrOrStderr(), "\ntry:  wootctl conversations list --status open")
				return nil
			},
		}
	})
